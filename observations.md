...
- 0x1: track numerics
  - offset 0, 2 bytes: another counter ascending by 2 starting from 1003
  - offset 8, 8 bytes: an ID, doesn't appear anywhere else
  - offset 60, 4 bytes or maybe 8 bytes: some values shared by thousands of tracks, but don't appear anywhere else
  - offset 76, 2 bytes: number of artworks
  - offset 84, 4 bytes: total file size of all artworks in bytes
  - offset 124, 4 bytes: 0 if no custom lyrics, otherwise seems to be some kind of tiny hash because having the same lyrics always produces the same result. However, the actual lyrics are only stored embedded in the audio file.
...
- 0xca: smart playlist options
  ...
  - offset 0, 1 byte: live updating checkbox (0 = false, 1 = true)
  - offset 1, 1 byte: checkbox to enable matching rules (related data is still stored even when disabled)
    - GUI enforces that one must have either this or the limit (offset 22) enabled
  - "limit to \<count\> \<units\> selected by \<method\>"
    - offset 2, 1 byte: checkbox to enable the limit (related data is still stored even when disabled)
    - offset 3, 1 byte: units
      - 0x03 = items
      - 0x01 = minutes
      - 0x04 = hours
      - 0x02 = megabytes
      - 0x05 = gigabytes
    - offset 4, 1 byte: selection method
      - 0x02 = random
      - 0x06 = album
      - 0x07 = artist
      - 0x09 = genre
      ...
      - offset 13, 1 byte: reverse flag — when set to 0x01 instead of 0x00:
  ...
  - offset 12: match only checked items checkbox (note: to be able to toggle this in the GUI, must have "Settings" → "General" → "Show" → "Song list checkboxes" checked; the checkboxes this refers to can be seen by changing "View as" to "Songs" (not in any other view))
- 0xc9: smart playlist rules
  ...
  - starting at offset 20, contains "SLst" subsections
    - _AHA! This is why boma sections always have 0x14 00 00 00 at offset 4! This is in fact the "section length" of boma, and offset 8 is the "associated sections length" — therefore boma is not actually an unusual special case as all previous investigators have believed, but in fact following the usual pattern exactly! Even the subtype value matches the hsma sections, which also have a subtype at offset 12 that give a hint about what's inside! Instead, it is the sections_ inside _of the boma sections that often break the patterns:_
      ...
      - this was partially acknowledged by Gary Vollink in the case of ipfa under boma sections
    ...
    - offset 11, 4 bytes: number of rules contained (are these subsections too?), possibly including other SLst sections (for playlist folders: number of children — each is a rule "playlist is..." under a "match _any_ of the following")
...
todo check for the IDs in the preferences folder which are 8 bytes